    ...
    return new Promise((resolve, reject) => {
      ...
      const actualCall = (cb: Function) => {
        ...
        try {
          ...
          //  headers.Authorization = this._webhook.options.token;
          //}
          if (file) {
          ...
          if (this.latencyRef.lastTimeOffsetCheck < Date.now() - 5000) {
            ...
            if (
              this.latencyRef.timeOffset - this.latencyRef.latency >=
                (this._webhook.options.latencyThreshold as number) &&
              timeOffset - this.latencyRef.latency >= (this._webhook.options.latencyThreshold as number)
            ) {
              this._webhook.emit(
                'warn',
                new Error(
            ...
            .once('end', () => {
              ...
              if ((resp.statusCode as number) >= 300) {
                if (resp.statusCode === 429) {
                  ...
                  this._webhook.emit('debug', 'A wild 502 appeared! Thanks CloudFlare!');
                  setTimeout(() => {
                    this.request(method, url, auth, body, file, reason, route, true).then(resolve).catch(reject);
